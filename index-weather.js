...
const weather_min_max_temp_p = document.querySelector(".minmaxtempDisplay")
...
async function auto_fetch_api(){
    ...
    try{
        ...
        data_setter(await fetch_weather_data(response_ip_api.cityName));
        ...
        weather_search_button.style.display="inline"
            ...
            trying_for_data_frame.innerHTML=
            "<p>COULD FETCH FOR DATA, TRY A NEARBY CAPITAL 😌🙏 THANKYOU</p>";
            console.error(error);
        }
...
function data_setter(weather_json){
    ...
    `FEELS LIKE : ${Number(((weather_json.main.feels_like)-273).toFixed(1))} °C`;


    weather_min_max_temp_p.innerHTML=
    `MIN : ${Number(((weather_json.main.temp_min)-273).toFixed(1))} °C 
